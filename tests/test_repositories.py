...
class TestUserRepositoryFixed:
    ...
    def test_create_user_duplicate_email(self, db):
        ...
        # Create first user
        user1 = User(
            email="test@example.com", username="user1", hashed_password="hashed_pass1"
        )
        user_repo.create(user1)

        # Try to create user with same email
        user2 = User(
            email="test@example.com", username="user2", hashed_password="hashed_pass2"
        )
    ...
    def test_get_by_id_wrong_user(self, db):
        ...
        # Create users
        user1 = User(
            email="user1@example.com", username="user1", hashed_password="pass"
        )
        user2 = User(
            email="user2@example.com", username="user2", hashed_password="pass"
        )
        ...
        result = category_repo.get_by_id(
            category.id
        )  # Repository methods typically don't filter by user
        assert (
            result is not None
        )  # Category exists but access control is at service layer
...
class TestExpenseRepositoryFixed:
        ...
        for data in expenses_data:
            expense = Expense(
                ...
                category_id=test_category[
                    "id"
                ],  # Use dict access if test_category is also a dict
                **data,
            )
    ...
    def test_get_expense_stats_no_expenses(self, db, test_user):
        ...
        stats = expense_repo.get_expense_stats(
            test_user["id"], currency="USD"
        )  # Use dict access
    ...
    def test_get_category_stats_no_expenses(self, db, test_user):
        ...
        stats = expense_repo.get_category_stats(
    ...
    def test_get_monthly_expenses_no_expenses(self, db, test_user):
        ...
        expenses = expense_repo.get_monthly_expenses(
            test_user["id"], 2024, 1
        )  # Use dict access
