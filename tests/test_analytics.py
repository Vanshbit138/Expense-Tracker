...
class TestAnalyticsEdgeCases:
    ...
    def test_get_expense_stats_with_date_range(self, client, auth_headers, test_expense_data):
    ...
    def test_get_expense_stats_different_currency(self, client, auth_headers):
        ...
        response = client.get(
            "/api/v1/analytics/stats?currency=EUR",
            headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_category_stats_with_limit(self, client, auth_headers, test_expense_data):
        """Test category stats with limit parameter."""
        # Create expense
        client.post("/api/v1/expenses/", json=test_expense_data, headers=auth_headers)
        
        response = client.get(
            "/api/v1/analytics/category-stats?limit=5",
            headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_monthly_analytics_future_month(self, client, auth_headers):
        ...
        response = client.get(
            "/api/v1/analytics/monthly/2025/12",
            headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_monthly_analytics_invalid_year(self, client, auth_headers):
        ...
        response = client.get(
            "/api/v1/analytics/monthly/abc/1",  # Invalid year
            headers=auth_headers
        )
